...
class Generator(nn.Module):
    def __init__(
        ...
        self.swin_enc = SwinTransformerV2()
        ...
        fuser_0 = ContentLayerDeepFast(len=4, inp=1536, out=self.fuser_out_dim)
        fuser_1 = ContentLayerDeepFast(len=4, inp=1024, out=self.fuser_out_dim)
        fuser_2 = ContentLayerDeepFast(len=4, inp=768, out=self.fuser_out_dim)
        fuser_3 = ContentLayerDeepFast(len=4, inp=640, out=self.fuser_out_dim)
        self.fusers.append(fuser_0)
        self.fusers.append(fuser_1)
        self.fusers.append(fuser_2)
        self.fusers.append(fuser_3)
        
        self.feat2w = feat2wspace()
    ...
    def load_encoder(self):
        swinv2_path = "./pretrained_models/swinv2_base_patch4_window12to16_192to256_22kto1k_ft.pth"
        swinv2_weights = torch.load(swinv2_path)
        self.swin_enc.load_state_dict(swinv2_weights['model'])
        print('Pretrained weights of Image Encoder are loaded.')
        
    def extract_feat(self, images):
        _, styles, feats = self.swin_enc(images)
        return feats
    ...
    def forward(
        ...
        _, swin_feat, feat_stack = self.swin_enc(images)
        # styles = self.feat2w(swin_feat)
        
        # styles_list = []
        # resolutions = ['4x4', '8x8', '16x16', '32x32', '64x64', '128x128', '256x256']

        # for resolution in resolutions:
        #     swin_style = self.style_transforms[resolution](swin_feat)     # styles.shape: ([4, 512])
        #     styles_list.append(swin_style)
        #     styles_list.append(swin_style)      # 두 번씩 추가하여 2개씩 반복

        # # 스타일 벡터를 concat하고 truncation 적용
        # # style_features = torch.cat(styles_list, dim=1)
        # styles = torch.stack(styles_list, dim=1)
        
        # feats = []
        # feats.append(self.linear_0(feat_stack[0]))
        # feats.append(self.linear_1(feat_stack[1]))
        # feats.append(self.linear_2(feat_stack[2]))
        # feats.append(self.linear_3(feat_stack[3]))
        
        ori_styles = self.style(noise)
        styles = ori_styles
        ...
        ori_latent = ori_styles.unsqueeze(1).repeat(1, inject_index, 1)
        x = self.input(latent)
        B, C, H, W = x.shape
        ...
        for layer, to_rgb in zip(self.layers, self.to_rgbs):
            if count > 0:
                try:
                    feat = torch.cat([feat_stack[int(4-count/2)], x], dim=2)
                    feat_b, feat_n, feat_c = feat.shape
                    feat_h, feat_w = int(math.sqrt(feat_n)), int(math.sqrt(feat_n))
                    feat_reshape = feat.transpose(-1, -2).reshape(feat_b, feat_c, feat_h, feat_w)
                    fuser = self.fusers[int(count/2)-1]
                    x = fuser(feat_reshape)
                    x = x.reshape(feat_b, feat_n, self.fuser_out_dim)
                except:
                    pass
            x = layer(x, latent[:,count,:], latent[:,count+1,:])
            b, n, c = x.shape
            h, w = int(math.sqrt(n)), int(math.sqrt(n))
            skip = to_rgb(x.transpose(-1, -2).reshape(b, c, h, w), skip=skip)
            count = count + 2
        ...
        if return_latents:
            return image, latent, feat_stack
        else:
            return image, feat_stack, None
