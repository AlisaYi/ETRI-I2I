...
class ToRGB(nn.Module):     # 입력 이미지를 RGB 이미지로 변환
...
class Mlp(nn.Module):       # MLP 정의, Transformer 블록 내에서 사용
...
class WindowAttention(nn.Module):       # Swin Transformer에서 사용되는 window 기반 Multi head self attention
...
class AdaptiveInstanceNorm(nn.Module):      # Adaptive Instance Normalization(AdaIN)을 정의, 스타일 정보를 입력에 적용
...
class StyleSwinTransformerBlock(nn.Module):     # StyleSwin의 기본 Transformer 블록을 정의, 스타일 정보를 입력으로 받아 각 레이어에서 적절히 처리
...
class StyleBasicLayer(nn.Module):       # Swin Transformer 블록을 여러 단계로 쌓아 구성된 레이어를 정의, 스타일 정보를 각 블록에 전달하여 이미지 생성 과정을 지원
...
class BilinearUpsample(nn.Module):      # 양방향 업샘플링을 수행하는 클래스, 이미지 해상도를 높이는 데 사용됨
...
class Generator(nn.Module):
    def __init__(
        ...
        ### ViT 모델 로드 (기본 ViT 모델, img 크기 224x224, patch 크기 16x16.)
        self.vit_model = timm.create_model('vit_base_patch16_224', pretrained=True)
        self.vit_model.to(self.device)

        ### ViT 모델의 출력 차원에 맞는 스타일 변환 레이어
        self.style_transform = nn.Sequential(
            nn.Linear(style_dim, style_dim),  # ViT-base의 출력 차원 가정
            nn.ReLU(),
    ...
    def forward(
        ...
        # ViT 특징 추출
        with torch.no_grad():
            resized_images = F.interpolate(images, size=(224, 224), mode='bilinear', align_corners=False).to(self.device)
            vit_features = self.vit_model(resized_images)


        # 스타일 변환 (필요에 따라 추가 레이어를 사용하여 차원 조정)
        style_features = self.style_transform(vit_features.float())
        ...
        if truncation < 1:
            ...
            style_features = torch.cat(style_t, dim=0)
        
        if style_features.ndim < 3:
            latent = style_features.unsqueeze(1).repeat(1, inject_index, 1)
        else:
            latent = style_features
...
class Generator_MultiResolution(nn.Module):
    def __init__(
        self,
        size,
        style_dim,
        ...
        ### ViT 모델 로드 (기본 ViT 모델, img 크기 224x224, patch 크기 16x16.)
        self.vit_model = ViT(image_size=size, patch_size=16, num_classes=1000, dim=1024, depth=6, heads=16, mlp_dim=2048, dropout=0.1, emb_dropout=0.1)
        self.vit_model.to(self.device)
        ...
        # 해상도별 스타일 변환 레이어 정의
        self.style_transforms = nn.ModuleDict({
            '4x4': nn.Sequential(
                nn.Linear(1024, style_dim),  # ViT의 특징 차원을 style_dim(512)으로 변환
                nn.ReLU(),
                nn.Linear(style_dim, style_dim)
            ),
            '8x8': nn.Sequential(
            ...
            '256x256': nn.Sequential(
                nn.Linear(1024, style_dim),
                nn.ReLU(),
                nn.Linear(style_dim, style_dim)
            ),
            '512x512': nn.Sequential(
                nn.Linear(1024, style_dim),
                nn.ReLU(),
                nn.Linear(style_dim, style_dim)
            ),
            '1024x1024': nn.Sequential(
                nn.Linear(1024, style_dim),
                nn.ReLU(),
                nn.Linear(style_dim, style_dim)
            )
    ...
    def forward(
        ...
        #inject_index = self.n_latent   # n_latent: 전체 스타일 벡터 수

        # ViT 특징 추출
        # with torch.no_grad():
        #     resized_images = F.interpolate(images, size=(224, 224), mode='bilinear', align_corners=False).to(self.device)
        #     vit_features = self.vit_model.forward_features(resized_images)
        #     vit_feat_pool = self.avgpool(vit_features.transpose(1, 2))
        #     vit_feat_flat = torch.flatten(vit_feat_pool, 1)

        vit_features, _ = self.vit_model(images)
        
        # 해상도별 스타일 벡터 생성
        styles_list = []
        # resolutions = ['4x4', '8x8', '16x16', '32x32', '64x64', '128x128', '256x256', '512x512', '1024x1024']
        resolutions = ['4x4', '8x8', '16x16', '32x32', '64x64', '128x128', '256x256']

        for resolution in resolutions:
            style_transform = self.style_transforms[resolution]
            styles = style_transform(vit_features)     # styles.shape: ([4, 512])
            styles_list.append(styles)
            styles_list.append(styles)      # 두 번씩 추가하여 2개씩 반복

        # 스타일 벡터를 concat하고 truncation 적용
        # style_features = torch.cat(styles_list, dim=1)
        style_features = torch.stack(styles_list, dim=1)
        ...
        if truncation < 1:
            ...
            # style_features = torch.cat(style_t, dim=0)
            style_features = torch.stack(style_t, dim=0)

        # 각 레이어에 맞는 스타일 벡터 concat
        inject_index = len(self.layers) * 2

        ### repeat 대신 concat하여 latent 형상 맞추기
        if style_features.ndim < 3:
            latent = style_features.unsqueeze(1).repeat(1, inject_index, 1)
        else:
            latent = style_features
        ...
        #image = self.to_rgb_last(x.transpose(-1, -2).reshape(b, c, h, w), skip)
        B, L, C = x.shape
        assert L == self.size * self.size
        x = x.reshape(B, self.size, self.size, C).permute(0, 3, 1, 2).contiguous()
        image = skip
