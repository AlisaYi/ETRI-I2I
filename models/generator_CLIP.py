...
class AdaptiveInstanceNorm(nn.Module):      # Adaptive Instance Normalization을 정의, 스타일 정보를 입력에 적용
...
class Generator(nn.Module):
    def __init__(
        ...
        # CLIP 모델의 모든 파라미터를 고정 (freeze-학습 중에 업데이트 방지)
        ...
        '''
        ##### CLIP 모델의 출력 차원 사용
        self.style_dim = self.clip_model.visual.output_dim 

        ##### CLIP을 사용한 스타일 변환을 위한 추가 레이어
        # CLIP 모델로부터 추출된 이미지 특징을 입력으로 받아, 필요한 스타일 벡터 차원으로 변환
    ...
    def forward(
        ...
        ### 이미지 전처리 (필요한 경우, CLIP의 preprocess를 사용)
        # preprocessed_images = self.preprocess(images).to(self.device)

        ### CLIP을 통해 이미지 특징 추출
        with torch.no_grad():
            resized_images = F.interpolate(images, size=(224, 224), mode='bilinear', align_corners=False).to(self.device)
            image_features = self.clip_model.encode_image(resized_images)    

        ### 추출된 이미지 특징을 스타일 벡터로 변환
        pre_style = self.style_transform(image_features.float())
        ...
        if truncation < 1:
            style_t = []
            for style in pre_style:
                style_t.append(
                    truncation_latent + truncation * (style - truncation_latent)
                )

            pre_style = torch.cat(style_t, dim=0)
        
        if pre_style.ndim < 3:
            latent = pre_style.unsqueeze(1).repeat(1, inject_index, 1)
        else:
            latent = pre_style
    ...
    # 이미지를 생성할 때 발생하는 총 연산량(FLOPs, Floating Point Operations) 계산
    def flops(self):
        flops = 0
...
class Generator_MultiResolution(nn.Module):
    def __init__(
        ...
        self.style_transforms = nn.ModuleDict({
            '4x4': nn.Sequential(
                nn.Linear(self.style_dim, style_dim * 4),
                nn.ReLU(),
    ...
    def forward(
        ...
        for resolution in resolutions:
            ...
            styles = style_transform(image_features.float())
