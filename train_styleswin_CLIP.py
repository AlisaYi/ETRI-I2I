...
#from models.generator_CLIP import Generator
from models.generator_CLIP import Generator_MultiResolution
...
def tensor_transform_reverse(image):
    # normalization 역변환 (0~1 사이로 클램핑)
    return (image * torch.tensor([0.229, 0.224, 0.225], # 표준편차
                                 device=image.device).view(1, 3, 1, 1)) + torch.tensor([0.485, 0.456, 0.406],   # 평균
                                                                                       device=image.device).view(1, 3, 1, 1)    # 1차원 텐서를 (1, 3, 1, 1) 형태로 변환
# 정규화 역변환 수행: 
'''
...
if __name__ == "__main__":
    ...
    ### 모델 이름을 인수로 추가
    parser.add_argument('--model_name', type=str, default='CLIP', help='Name of the model to use')
    ### recon_weight 추가
    parser.add_argument("--recon_weight", type=float, default=10.0, help="Weight for the reconstruction loss")
